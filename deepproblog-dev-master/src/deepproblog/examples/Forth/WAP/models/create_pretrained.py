...
def train(label_list, text_list, context_list, index_list):
    ...
    for idx in range(label_list.shape[0]):
        optimizer.zero_grad()
        predicted_label = nn_wc(text_list[idx], context_list[idx], index_list[idx])
        predicted_label = predicted_label.unsqueeze(dim=0)
        loss = criterion(predicted_label, label_list[idx].view(1))
        loss.backward()
        torch.nn.utils.clip_grad_norm_(nn_wc.parameters(), 0.1)
        optimizer.step()
...
def evaluate(label_list, text_list, context_list, index_list):
    nn_wc.eval()
    total_acc, total_count = 0, len(label_list)
...
def get_instances(dataset):
    ...
    label_list, text_list, context_list, index_list = [], [], [], []
    for l in file1:
        line = l.split(',')
        label_list.append(int(line[-1]))
        index_list.append(line[-2])
        line = l.split(']')
        text = line[0]
        cont = line[1]
        sent = []
    ...
    return label_list, text_list, context_list, index_list          
    
    
   
nn_wc = WC(len(vocab), hidden_size, tagset_size) #MNIST_Net(with_softmax=False)
label_list, text_list, context_list, index_list  = get_instances("pretrain_wc.txt")
criterion = torch.nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(nn_wc.parameters(), lr=1e-3)
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 1.0, gamma=0.1)
total_accu = None
for epoch in range(1, 5):
    epoch_start_time = time.time()
    train(label_list[0:50], text_list[0:50], context_list[0:50], index_list[0:50])
    accu_val = evaluate(label_list[50:], text_list[50:], context_list[50:], index_list[50:])
    ...
    print('| end of epoch {:3d} | time: {:5.2f}s | '
          'valid accuracy {:8.3f} '.format(epoch,
                                           ...
                                           accu_val))
    print('-' * 59)
torch.save(nn_wc.state_dict(), "{}_{}.pth".format('1_5', 'nn_wc_roberta_384_2'))
