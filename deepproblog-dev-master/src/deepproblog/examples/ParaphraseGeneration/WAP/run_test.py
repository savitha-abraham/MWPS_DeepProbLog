...
#from deepproblog.examples.ParaphraesGeneration.WAP.Pretraining import get_networks,VC, WC
from genWAP_Net import get_networks, QC, NC
...
##name = '20PD_' + format_time_precise()
...
#Pretraining
nn_n0.load_state_dict(torch.load("snapshot/nn_n0_PD.pth"))
nn_query.load_state_dict(torch.load("snapshot/nn_query_PD.pth"))
...
#SAVE NEURAL: Also save the neural network states separately 
...
##model.load_state('snapshot/20FS_220428_163745841880.pth')
print('Test Set Performance Normal..')

print(get_confusion_matrix(model, test_queries))

acc_test_n = get_confusion_matrix(model, test_queries).accuracy()
